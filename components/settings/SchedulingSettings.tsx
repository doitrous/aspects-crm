...
function bookingPlatformVisibility(
  schedule: Pick<CrmScheduleRow, "active" | "showOnBookingWebsite" | "doctorId" | "branchId">,
  snapshot: CrmSchedulingSnapshot,
) {
  if (!schedule.active) return { seen: false, reason: "This schedule is inactive." };
  if (!schedule.showOnBookingWebsite) return { seen: false, reason: "Public visibility is switched off." };
  const doctor = snapshot.doctors.find((item) => item.id === schedule.doctorId);
  if (!doctor?.active) return { seen: false, reason: "The doctor is inactive in the booking catalog." };
  const branch = snapshot.branches.find((item) => item.id === schedule.branchId);
  if (!branch?.active) return { seen: false, reason: "The branch is inactive." };
  if (!branch.publicBooking) return { seen: false, reason: "The branch is hidden from public booking." };
  return { seen: true, reason: "Patients can select this schedule in online booking." };
}

function BookingPlatformNote({
  schedule,
  snapshot,
}: {
  schedule: Pick<CrmScheduleRow, "active" | "showOnBookingWebsite" | "doctorId" | "branchId">;
  ...
  const visibility = bookingPlatformVisibility(schedule, snapshot);
  return (
    <div className={`crm-booking-visibility ${visibility.seen ? "is-seen" : "is-hidden"}`} role="note">
      <strong><span aria-hidden="true">{visibility.seen ? "●" : "○"}</span> {visibility.seen ? "Seen on booking platform" : "Not seen on booking platform"}</strong>
                  ...
                  return (
                    <div key={day.value} className={`crm-day-card ${dayOff ? "is-off" : ""}`}>
                          ...
                          <article key={schedule.id} className={`crm-session-card ${scheduleCompletionIssues(schedule, snapshot).length ? "is-incomplete" : ""}`}>
                            <div className="flex items-start justify-between gap-1">
                              <div>
                                ...
                                <BookingPlatformNote schedule={schedule} snapshot={snapshot}/>
                      ...
                      <div className="mt-3 space-y-1.5">
                        {inactiveRows.map((schedule) => <article key={schedule.id} className="crm-inactive-session"><BookingPlatformNote schedule={schedule} snapshot={snapshot}/><div className="mt-1.5 flex items-center gap-1"><button type="button" onClick={() => setEditor({ schedule })} className="crm-day-action flex-1">Restore {schedule.startTime}</button><DeleteScheduleButton schedule={schedule} doctorName={doctor.nameEn} dayLabel={day.label}/></div></article>)}
  ...
  return (
    <div className="space-y-5">
      <section className="crm-schedule-panel">
        <div className="crm-schedule-panel-heading"><div><h3>Clinic rooms</h3><p>Canonical rooms managed here for CRM scheduling and online availability.</p></div><div className="flex flex-wrap gap-2"><form action={syncAction}><button disabled={syncing} className="crm-schedule-button secondary">{syncing ? "Refreshing…" : "↻ Refresh shared rooms"}</button></form><button type="button" onClick={() => setEditor("new")} className="crm-schedule-button primary">＋ Add room</button></div></div>
