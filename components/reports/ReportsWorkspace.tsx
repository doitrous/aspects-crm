...
function AutomaticSnapshot({ auto }: { auto: ReportAutoData }) {
  ...
  return <section><div className="mb-2 flex items-center justify-between"><div><div className="text-[10px] font-black uppercase tracking-[0.14em] text-primary">Step 2 · automatic facts</div><p className="mt-0.5 text-[10.5px] text-ink-400">Already calculated from live CRM data for {auto.date}. No extra Calculate button is needed.</p></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[9.5px] font-black text-emerald-700">LIVE DATA LOADED</span></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{values.map(([label, value]) => <div key={String(label)} className="rounded-xl border border-line bg-slate-50 p-3"><div className="text-[9.5px] font-bold uppercase tracking-wide text-ink-400">{label}</div><div className="mt-1 text-[19px] font-black text-ink-900">{value}</div></div>)}</div></section>;
}
...
function Creator({ date, role, auto }: { date: string; role: Role; auto: ReportAutoData }) {
  ...
  return <section className="overflow-hidden rounded-xl border border-line bg-panel shadow-sm">
    ...
    <form action={action} className="space-y-5 p-4 sm:p-5">
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-line pt-4"><button disabled={pending} className="h-10 rounded-control bg-ink-900 px-5 text-[12px] font-black text-white hover:bg-primary disabled:opacity-50">{pending ? "Finalizing…" : "Review complete · Finalize report"}</button><p className="text-[10.5px] text-ink-400">Finalization creates a saved, shareable copy. It does not change the CRM source data.</p></div>
...
export function ReportsWorkspace({ reports, selectedId, date, auto, scorecards, role, warning }: { reports: SummaryReport[]; selectedId?: string; date: string; auto: ReportAutoData; scorecards: ModeratorScorecard[]; role: Role; warning?: string }) {
  ...
  return <main className="min-h-0 flex-1 overflow-auto bg-canvas p-3 sm:p-[18px]"><div className="mx-auto max-w-[1500px] space-y-4">
    <header className="rounded-xl bg-ink-900 p-5 text-white"><div className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-[0.16em] text-white/50">Operational reporting</div><h1 className="mt-1 text-[26px] font-black">Reports with a clear purpose</h1><p className="mt-1 max-w-2xl text-[12px] text-white/60">Automatic CRM facts, human context, one intentional finalization step, and an image-ready saved output.</p></div><form method="get" className="flex items-end gap-2"><label className="text-[10px] font-bold text-white/60">Working date<input type="date" name="date" defaultValue={date} className="mt-1 block h-9 rounded-control border border-white/20 bg-white/10 px-3 text-[12px] text-white" /></label><button className="h-9 rounded-control bg-white px-3 text-[11px] font-black text-ink-900">Load date</button></form></div></header>
    ...
    {selected && meta ? <section className="grid gap-4 lg:grid-cols-[260px_1fr]"><aside className="space-y-2 rounded-xl border border-line bg-panel p-3"><div className="px-1 pb-2 text-[10px] font-black uppercase tracking-[0.14em] text-ink-400">Finalized reports</div>{reports.map((report) => { const item = META[report.reportType] ?? meta; const active = report.id === selected.id; return <Link key={report.id} href={`/reports?id=${report.id}&date=${report.reportDate}`} className="block rounded-xl border p-3" style={{ borderColor: active ? item.color : "#e4e7ec", background: active ? item.soft : "white" }}><div className="text-[11px] font-black" style={{ color: item.color }}>{item.label}</div><div className="mt-1 text-[12px] font-bold text-ink-800">{report.reportDate}</div><div className="mt-0.5 text-[10px] text-ink-400">{report.generatedBy || "CRM"}</div></Link>; })}</aside><div className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ background: meta.soft, color: meta.color }}>{meta.label}</span><span className="ml-2 text-[10.5px] text-ink-400">Finalized · {selected.reportDate}</span></div><ExportButtons text={selected.text} target={reportRef} /></div><div className="overflow-hidden rounded-xl border border-line bg-white shadow-sm"><div ref={reportRef} className="bg-white"><div className="h-3" style={{ background: meta.color }} /><div className="p-7 sm:p-10"><div className="mb-7 flex items-start justify-between gap-5 border-b-2 border-ink-900 pb-5"><div><div className="text-[11px] font-black uppercase tracking-[0.2em]" style={{ color: meta.color }}>Aspects Clinica</div><div className="mt-2 text-[30px] font-black leading-tight text-ink-900">{meta.label}</div><div className="mt-1 text-[13px] text-ink-500">{meta.purpose}</div></div><div className="text-right"><div className="text-[15px] font-black text-ink-900">{selected.reportDate}</div><div className="mt-1 text-[10px] font-black uppercase tracking-wide" style={{ color: meta.color }}>Final report</div></div></div><FormattedReport text={selected.text} /><div className="mt-8 border-t border-line pt-4 text-[10px] font-semibold text-ink-400">Generated from Aspects Clinica CRM · finalized operational record</div></div></div></div></div></section> : <div className="rounded-xl border border-line bg-panel p-10 text-center text-[13px] text-ink-500">No finalized reports yet. Use the guided flow above to create the first one.</div>}
  </div></main>;
}
