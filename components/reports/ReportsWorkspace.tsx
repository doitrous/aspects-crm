...
export function ReportsWorkspace({ reports, selectedId, date, auto, scorecards, role, warning }: { reports: SummaryReport[]; selectedId?: string; date: string; auto: ReportAutoData; scorecards: ModeratorScorecard[]; role: Role; warning?: string }) {
  ...
  return <main className="min-h-0 flex-1 overflow-auto bg-canvas p-3 sm:p-[18px]"><div className="mx-auto max-w-[1500px] space-y-4">
    <header className="section-hero rounded-xl p-5"><div className="flex flex-wrap items-end justify-between gap-4"><div><div className="section-hero-eyebrow text-[10px] font-black uppercase tracking-[0.16em]">Operational reporting</div><h1 className="mt-1 text-[26px] font-black text-ink-950">Reports with a clear purpose</h1><p className="section-hero-muted mt-1 max-w-2xl text-[12px]">Automatic CRM facts, human context, one intentional finalization step, and an image-ready saved output.</p></div><form method="get" className="flex items-end gap-2"><label className="text-[10px] font-bold text-ink-500">Working date<input type="date" name="date" defaultValue={date} className="mt-1 block h-9 rounded-control border border-line bg-panel px-3 text-[12px] text-ink-900" /></label><button className="h-9 rounded-control bg-primary px-3 text-[11px] font-black text-white hover:bg-primary-hover">Load date</button></form></div></header>
