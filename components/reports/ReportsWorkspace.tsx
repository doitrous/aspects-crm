...
function Creator({ date, role }: { date: string; role: Role }) {
  ...
  const types: ReportType[] = all ? ["moderator_daily_ar","follow_up_daily_ar","auditor_clinic_daily_ar","financial_daily"] : ["moderator_daily_ar","follow_up_daily_ar"];
...
export function ReportsWorkspace({reports,selectedId,date,auto,scorecards,role}:{reports:SummaryReport[];selectedId?:string;date:string;auto:ReportAutoData;scorecards:ModeratorScorecard[];role:Role}) {
  const selected=useMemo(()=>reports.find(r=>r.id===selectedId)??reports[0],[reports,selectedId]); const reportRef=useRef<HTMLDivElement>(null); const m=selected?META[selected.reportType]??{label:selected.reportType,color:"#475467",soft:"#f2f4f7",icon:"▦"}:null;
  return <div className="min-h-0 flex-1 overflow-auto bg-canvas px-[18px] py-4"><div className="mx-auto flex max-w-[1500px] flex-col gap-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-[20px] font-black text-ink-900">Reports that are ready to share</h1><p className="text-[12px] text-ink-500">Create, review, compare and export operational reports from one place.</p></div><form><input type="date" name="date" defaultValue={date} className={input}/></form></div><Creator date={date} role={role}/><Scores date={date} rows={scorecards}/>{selected && m ? <div className="grid gap-4 lg:grid-cols-[270px_1fr]"><aside className="space-y-2">{reports.map(r=>{const rm=META[r.reportType]??m;return <Link key={r.id} href={`/reports?id=${r.id}&date=${date}`} className={`block rounded-xl border p-3 ${r.id===selected.id?"shadow-sm":"bg-white"}`} style={r.id===selected.id?{borderColor:rm.color,background:rm.soft}:{}}><span className="text-[10px] font-black" style={{color:rm.color}}>{rm.icon} {rm.label}</span><div className="mt-1 text-[12px] font-bold">{r.reportDate}</div><div className="text-[10px] text-ink-400">{r.generatedBy||"CRM"}</div></Link>})}</aside><div className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><span className="rounded-full px-2.5 py-1 text-[10.5px] font-black" style={{background:m.soft,color:m.color}}>{m.icon} {m.label}</span><span className="ml-2 text-[11px] text-ink-500">Finalized · {selected.reportDate}</span></div><ExportButtons text={selected.text} target={reportRef}/></div><Card className="overflow-hidden"><div ref={reportRef} className="bg-white"><div className="h-2" style={{background:`linear-gradient(90deg,${m.color},#7c3aed)`}}/><div className="p-6"><div className="mb-5 flex items-start justify-between border-b border-line-soft pb-4"><div><div className="text-[11px] font-black uppercase tracking-[.18em]" style={{color:m.color}}>Aspects Clinica</div><h2 className="mt-1 text-[22px] font-black text-ink-900">{m.label}</h2></div><div className="rounded-xl px-3 py-2 text-right text-[11px] font-bold" style={{background:m.soft,color:m.color}}>{selected.reportDate}<small className="block opacity-70">FINAL REPORT</small></div></div>{selected.reportType==="financial_daily"&&<div className="mb-5"><MiniCharts auto={auto}/></div>}<pre dir="auto" className="whitespace-pre-wrap font-sans text-[13px] leading-7 text-ink-800">{selected.text}</pre></div></div></Card></div></div>:<Card className="p-10 text-center text-[13px] text-ink-500">No reports yet. Create the first report above.</Card>}</div></div>;
}
