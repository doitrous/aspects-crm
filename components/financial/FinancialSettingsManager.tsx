...
import {
  ...
  deleteServiceConsumableDefaultAction,
...
function ServicePriceForm({ service, data }: { service: FinancialServiceSetting; data: FinancialSettingsData }) {
  const [state, action, pending] = useActionState(upsertServicePriceAction, IDLE);
  const hasCompensation = data.doctorCompRules.some((rule) => (rule.serviceId && rule.serviceId === service.serviceId) || rule.serviceName === service.serviceName);
  const [compensationMode, setCompensationMode] = useState<"keep" | "global" | "doctors">(hasCompensation ? "keep" : "global");
  ...
  return (
    <form action={action} className={"rounded-xl border p-3.5 " + (complete ? "border-line bg-white" : "border-amber-200 bg-amber-50/45")}>
      ...
      {compensationMode === "global" && data.doctors.doctors.filter((doctor) => doctor.active).map((doctor) => <input key={doctor.id} type="hidden" name="compensationDoctorRef" value={`${doctor.id}::${doctor.nameEn}`} />)}
      ...
      <fieldset className="sm:col-span-2 lg:col-span-4"><legend className="text-[11px] font-semibold text-ink-500">Specialties (a service may appear in more than one)</legend><div className="mt-1 flex flex-wrap gap-2">{data.doctors.specialties.map((specialty) => <label key={specialty.id} className="rounded-control border border-line-soft px-2 py-1 text-[10.5px] text-ink-700"><input className="me-1" type="checkbox" name="specialtyIds" value={specialty.id} defaultChecked={service.specialtyIds.includes(specialty.id) || service.specialtyId === specialty.id}/>{specialty.nameEn}</label>)}</div></fieldset>
      <div className="grid gap-2 rounded-control border border-primary/15 bg-primary-soft/20 p-3 sm:col-span-2 sm:grid-cols-2 lg:col-span-4 lg:grid-cols-4"><label className={label}>Doctor compensation<select name="compensationMode" value={compensationMode} onChange={(event) => setCompensationMode(event.target.value as typeof compensationMode)} className={field}>{hasCompensation && <option value="keep">Keep existing rules</option>}<option value="global">All active doctors</option><option value="doctors">Selected doctors</option></select></label>{compensationMode === "doctors" && <label className={`${label} lg:col-span-3`}>Doctors<CheckboxPicker name="compensationDoctorRef" options={data.doctors.doctors.map((doctor) => ({ value: `${doctor.id}::${doctor.nameEn}`, label: doctor.nameEn }))}/></label>}{compensationMode !== "keep" && <><label className={label}>Payout type<select name="compensationKind" className={field}><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option></select></label><label className={label}>Payout value<input name="compensationValue" required type="number" min="0" step="0.01" className={field}/></label><label className={label}>Quoted price / net<select name="compensationBasis" className={field}><option value="quoted_price">Quoted price</option><option value="net_after_consumables">Net after consumables</option></select></label></>}</div>
...
function Services({ data }: { data: FinancialSettingsData }) {
  ...
  const groups = useMemo(() => {
    ...
    for (const service of filtered) {
      const specialtyIds = service.specialtyIds.length ? service.specialtyIds : service.specialtyId ? [service.specialtyId] : [];
      const keys = specialtyIds.length ? specialtyIds.map((id) => data.doctors.specialties.find((specialty) => specialty.id === id)?.nameEn ?? "Unknown specialty") : ["All specialties"];
      for (const key of keys) grouped.set(key, [...(grouped.get(key) ?? []), service]);
    }
    return [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b));
  }, [data.doctors.specialties, filtered]);
...
function ConsumableDefaultRow({ item, data }: { item: FinancialSettingsData["serviceConsumableDefaults"][number]; data: FinancialSettingsData }) {
  const [open, setOpen] = useState<"duplicate" | "edit" | null>(null);
  const [state, action, pending] = useActionState(duplicateConsumableRuleAction, IDLE);
  const [editState, editAction, editing] = useActionState(upsertServiceConsumableDefaultAction, IDLE);
  const [deleteState, deleteAction, deleting] = useActionState(deleteServiceConsumableDefaultAction, IDLE);
  return <div className="mt-1 rounded-control border border-line-faint bg-panel/70 px-2.5 py-2 text-[11.5px] text-ink-600"><div className="flex flex-wrap items-center justify-between gap-2"><span>{item.description} · <b>{money(item.totalCost)} EGP</b> {item.active ? "" : "(inactive)"}</span><div className="flex gap-2"><button type="button" onClick={() => setOpen(open === "edit" ? null : "edit")} className="font-bold text-primary">Edit</button><button type="button" onClick={() => setOpen(open === "duplicate" ? null : "duplicate")} className="font-bold text-primary">Duplicate</button><form action={deleteAction} onSubmit={(event) => { if (!window.confirm(`Delete ${item.description} from ${item.serviceName}?`)) event.preventDefault(); }}><input type="hidden" name="id" value={item.id}/><button disabled={deleting} className="font-bold text-danger">Delete</button></form></div></div>{open === "edit" && <form action={editAction} className="mt-2 grid gap-2 border-t border-line-faint pt-2 sm:grid-cols-4"><input type="hidden" name="id" value={item.id}/><input type="hidden" name="serviceId" value={item.serviceId ?? ""}/><input type="hidden" name="serviceName" value={item.serviceName}/><label className={label}>Description<input name="description" defaultValue={item.description} className={field}/></label><label className={label}>Quantity<input name="quantity" type="number" min="0.001" step="0.001" defaultValue={item.quantity} className={field}/></label><label className={label}>Unit cost<input name="unitCost" type="number" min="0" step="0.01" defaultValue={item.unitCost} className={field}/></label><label className="pt-5 text-[11px]"><input name="active" type="checkbox" defaultChecked={item.active}/> Active</label><div className="sm:col-span-4"><Save pending={editing}>Save changes</Save><button type="button" onClick={() => setOpen(null)} className="ms-2 h-8 px-2">Cancel</button><Feedback state={editState}/></div></form>}{open === "duplicate" && <form action={action} className="mt-2 border-t border-line-faint pt-2"><input type="hidden" name="sourceId" value={item.id} /><p className="mb-2 text-[10.5px] text-ink-500">Copy quantity, unit cost and active state to one or more services.</p><CheckboxPicker name="targetServiceRef" options={data.services.map((service) => ({ value: serviceRef(service), label: service.serviceName }))} /><div className="mt-2 flex flex-wrap items-center gap-2"><Save pending={pending}>Duplicate to services</Save><button type="button" onClick={() => setOpen(null)} className="h-8 px-2 text-[11px]">Cancel</button><Feedback state={state} /></div></form>}<Feedback state={deleteState}/></div>;
}
...
function Consumables({ data }: { data: FinancialSettingsData }) {
  ...
  const specialtyGroups = useMemo(() => {
    ...
    for (const service of data.services) {
      const specialtyIds = service.specialtyIds.length ? service.specialtyIds : service.specialtyId ? [service.specialtyId] : [];
      const names = specialtyIds.length
        ? specialtyIds.map((id) => data.doctors.specialties.find((specialty) => specialty.id === id)?.nameEn ?? "Unknown specialty")
        : ["All specialties"];
      for (const name of names) grouped.set(name, [...(grouped.get(name) ?? []), service]);
    }
    return [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b));
  }, [data.doctors.specialties, data.services]);
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {specialtyGroups.map(([specialty, services]) => <section key={specialty}><h5 className="mb-2 border-b border-line-soft pb-2 text-[13px] font-black text-ink-900">{specialty}</h5><div className="grid gap-2 md:grid-cols-2">{services.map((service) => {
...
function StaffCommissions({ data }: { data: FinancialSettingsData }) {
  ...
  const globalRule = data.staffCommissionRules.find((rule) => rule.active && !rule.moderatorId && !rule.doctorId && !rule.serviceId && !rule.specialtyId);
  return <div className="space-y-4"><SettingsHero eyebrow="Commission logic" title="Moderator commissions with visible scope" description="Every active moderator is shown below. A global rule immediately becomes the effective percentage for every moderator." stats={[{ label: "Rules", value: data.staffCommissionRules.length }, { label: "Moderators", value: data.moderators.length }]} /><Card className="p-4"><div className="mb-3"><h3 className="text-[16px] font-black text-ink-950">Create a commission rule</h3><p className="text-[11.5px] text-ink-500">Leave every scope blank to apply one global percentage to all moderators.</p></div><form action={action} className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5 xl:items-end"><label className={label}>Moderator<select name="moderatorId" className={field}><option value="">All moderators</option>{data.moderators.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label className={label}>Doctor<select name="doctorId" className={field}><option value="">All doctors</option>{data.doctors.doctors.map((d) => <option key={d.id} value={d.id}>{d.nameEn}</option>)}</select></label><label className={label}>Specialty<select name="specialtyId" className={field}><option value="">All specialties</option>{data.doctors.specialties.map((s) => <option key={s.id} value={s.id}>{s.nameEn}</option>)}</select></label><label className={label}>Service<select name="serviceRef" className={field}><ServiceOptions services={data.services} /></select></label><label className={label}>Commission %<input name="commissionPct" type="number" min={0} max={100} step="0.01" required className={field} /></label><input type="hidden" name="active" value="true" /><div><Save pending={pending}>Apply rule</Save><Feedback state={state} /></div></form></Card><div className="grid gap-2 md:grid-cols-2">{data.moderators.map((moderator) => { const specific = data.staffCommissionRules.find((rule) => rule.active && rule.moderatorId === moderator.id && !rule.serviceId && !rule.specialtyId && !rule.doctorId); const effective = globalRule ?? specific; return <Card key={moderator.id} className="p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-[13px] font-black text-ink-900">{moderator.name}</div><div className="mt-1 text-[22px] font-black text-ink-950">{effective?.commissionPct ?? 0}%</div><div className="text-[10.5px] text-ink-500">{globalRule ? "Global rule applied" : specific ? "Moderator-specific rule" : "No active rule"}</div></div><span className="rounded-pill bg-primary-soft px-2 py-1 text-[10px] font-bold text-primary">Effective</span></div></Card>; })}</div><details className="rounded-card border border-line-soft bg-panel p-3"><summary className="cursor-pointer text-[12px] font-bold text-ink-700">View all scoped rules</summary><div className="mt-2 grid gap-2 md:grid-cols-2">{data.staffCommissionRules.map((r) => <div key={r.id} className="rounded-control border border-line-soft p-3"><div className="flex justify-between"><b>{r.commissionPct}%</b><ActiveToggle table="crm_staff_commission_rules" id={r.id} active={r.active}/></div><div className="mt-1 text-[10.5px] text-ink-500">{r.moderatorId ?? "All moderators"} · {r.serviceName ?? "All services"}</div></div>)}</div></details></div>;
}
...
function Doctors({ data }: { data: FinancialSettingsData }) {
  ...
  return (
    ...
    <Card className="p-4">
      ...
      <div className="grid gap-3 xl:grid-cols-2">{doctorsWithRules.map(({ doctor, rules }) => <Card key={doctor.id} className={`min-w-0 p-4 ${rules.length ? "" : "border-amber-200 bg-amber-50/40"}`}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className="flex h-12 w-12 flex-none items-center justify-center overflow-hidden rounded-full bg-primary-avatar text-[12px] font-black text-primary">{doctor.photoUrl ? <Image unoptimized loader={({ src }) => src} src={doctor.photoUrl} alt={`${doctor.nameEn} profile`} width={48} height={48} className="h-full w-full object-cover"/> : doctor.nameEn.split(/\s+/).slice(0,2).map((part) => part[0]).join("")}</div><div><h3 className="text-[16px] font-black text-ink-950">{doctor.nameEn}</h3><div className="mt-1 text-[11px] text-ink-500">{doctor.active ? "Active in booking catalog" : "Inactive / not bookable"}</div></div></div><span className={`rounded-pill px-2 py-1 text-[10px] font-black ${rules.length ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-800"}`}>{rules.length ? `${rules.length} rule${rules.length === 1 ? "" : "s"}` : "Needs payout rule"}</span></div><div className="mt-3 space-y-2">{rules.map((rule) => <DoctorCompRuleCard key={rule.id} rule={rule} data={data} />)}</div></Card>)}</div>
    </div>
  );
}
